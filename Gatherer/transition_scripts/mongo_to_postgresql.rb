...
class Relayer
  ...
  conn = PGconn.connect('dbname=tweet_app_db_2')
  content_array = []
  reply_no = 0
  counter = 0
         ...
         |reply| 
            ...
            reply.to_a[0].to_s.scan(simple_regex).each { 
                |entry|
                    ...
                    conn.prepare("#{counter}", 'insert into tweets ( content, "tweetId", replies, sarcastic, reply_1, reply_2, reply_3, reply_4, reply_5) values ($1, $2, $3, $4, $5, $6, $7, $8, $9)')
                    conn.exec_prepared("#{counter}", [ "#{(content_array[0])}","#{(content_array[-2])}", "#{(content_array[-1])}",
